...
import provider_shapenet as provider
import tf_util
...
os.system('cp train_shapenet_IoU.py %s' % (LOG_DIR)) # bkp of train procedure
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            iou = eval_scene_one_epoch(sess, ops, test_writer,NUM_CLASSES)
...
def train_scene_one_epoch(sess, ops, train_writer, num_classes, epoch):
    ...
    for batch_idx in range(num_batches):
        ...
        if (batch_idx+1)%10 == 0:
            log_string(' -- %03d / %03d --' % (batch_idx+1, num_batches))
            log_string('mean loss: %f' % (loss_sum / 10))
            #log_string('mean loss decomposed: %f; %f; %f' % (loss_classify/10, loss_cos/10, loss_scene/10))
            log_string('accuracy: %f' % (total_correct / float(total_seen)))
            total_correct = 0
            total_seen = 0
            loss_sum = 0
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    mean_shape_ious = np.mean(np.array([x for x in shape_ious.values()]))
